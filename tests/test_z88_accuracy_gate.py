...
def test_accuracy_gate_passes_with_expected_local_evidence(tmp_path: Path) -> None:
    ...
    structural = tmp_path / "z88_assets" / "outputs" / "structural_sample_method_validation.json"
    structural.write_text(
        json.dumps(
            {
                "results": [
                    {
                        "name": f"sample_{method}",
                        ...
                        "workflow": {
                            ...
                            "optimized_export": {"status": "exported"},
                        },
                    }
                    for method in ("oc", "toss", "sko")
                ]
            }
        ),
        encoding="utf-8",
